  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-[10vh] p-4">
      ...
      <motion.div
        ...
        className="relative w-full max-w-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-2xl overflow-hidden z-10 flex flex-col max-h-[75vh]"
        onKeyDown={handleKeyDown}
      >
        {/* Search Input Area */}
        <div className="flex items-center gap-3 px-4 py-3.5 border-b border-slate-150 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50">
          <Search className="w-5 h-5 text-slate-400 dark:text-slate-500 flex-shrink-0" />
          <input
            ref={inputRef}
            type="text"
            className="w-full bg-transparent text-sm text-slate-800 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none border-none p-0"
            placeholder="Type a command, task title, priority, or project..."
            value={query}
            onChange={(e) => setQuery(e.target.value)}
          />
          <kbd className="hidden sm:inline-flex px-1.5 py-0.5 text-[9px] font-mono font-bold bg-white dark:bg-slate-800 text-slate-400 border border-slate-200 dark:border-slate-700 rounded shadow-xs select-none">
            ESC
          </kbd>
        </div>
              ...
              {['Actions', 'Navigation', 'Tasks'].map(category => {
                ...
                return (
                  <div key={category} className="space-y-1">
                    <h4 className="px-2.5 pt-1.5 pb-1 text-[10px] font-bold text-slate-450 dark:text-slate-500 uppercase tracking-wider select-none font-mono">
                      {category}
                    </h4>
                          ...
                          return (
                            ...
                            >
                              <div className="flex items-center gap-3 min-w-0">
                                <span className={`p-1.5 rounded ${isSelected ? 'bg-white/20 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-500'}`}>
                                  {cmd.icon}
                                </span>
                                <div className="min-w-0">
                                  <p className="text-xs font-semibold truncate leading-none mb-1">{cmd.title}</p>
                                  <p className={`text-[10px] truncate ${isSelected ? 'text-indigo-150' : 'text-slate-400 dark:text-slate-500'}`}>{cmd.subtitle}</p>
                                </div>
                              </div>
                              {cmd.shortcut && (
                                <span className={`text-[9px] font-mono font-bold px-1.5 py-0.5 rounded select-none ${
                                  isSelected ? 'bg-white/20 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-400 border border-slate-200/50 dark:border-slate-750'
                                }`}>
                                  {cmd.shortcut}
                                </span>
                              )}
                            ...
                            >
                              <div className="flex items-center gap-3 min-w-0 flex-1">
                                ...
                                <div className="min-w-0 flex-1">
                                  ...
                                  <p className={`text-[10px] truncate ${isSelected ? 'text-indigo-150' : 'text-slate-400 dark:text-slate-500'}`}>
        ...
        <div className="bg-slate-50 dark:bg-slate-900/60 border-t border-slate-150 dark:border-slate-800 px-4 py-2.5 flex items-center justify-between text-[10px] text-slate-400 dark:text-slate-500 font-medium select-none">
          <div className="flex items-center gap-3">
            <span className="flex items-center gap-1">
              <kbd className="px-1 py-0.2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded text-[9px] font-mono">↑↓</kbd> navigate
            </span>
            <span className="flex items-center gap-1">
              <kbd className="px-1 py-0.2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded text-[9px] font-mono">Enter</kbd> select
            </span>
            <span className="flex items-center gap-1">
              <kbd className="px-1 py-0.2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded text-[9px] font-mono">ESC</kbd> close
            </span>
          </div>
