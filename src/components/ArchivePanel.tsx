...
export const ArchivePanel: React.FC<ArchivePanelProps> = ({
  ...
  return (
    <div className="space-y-6 font-sans">
      ...
      <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-3xs flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex items-center bg-slate-100 p-1 rounded-lg">
            ...
            >
              Archived Tasks ({tasks.filter(t => t.archived && !t.isDeleted && allowedDisciplines.includes(t.type)).length})
            </button>
            <button
              onClick={() => {
            ...
            {isAdmin && (
              ...
              >
                Deleted Tasks ({tasks.filter(t => t.isDeleted).length})
              </button>
            )}
          </div>
      ...
      <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-3xs flex flex-col lg:flex-row gap-3 items-stretch lg:items-center justify-between">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder={
              activeSubTab === 'tasks' 
                ? "Search archived tasks by code, title, or details..." 
                : activeSubTab === 'deleted'
        ...
        {(activeSubTab === 'tasks' || activeSubTab === 'deleted') && (
      ...
      ) : activeSubTab === 'deleted' && isAdmin ? (
        deletedTasks.length === 0 ? (
          <div className="bg-white border border-slate-200 rounded-xl p-12 text-center shadow-3xs">
            <Trash2 className="w-10 h-10 text-slate-300 mx-auto mb-3" />
            <p className="text-sm font-bold text-slate-800">No deleted tasks found</p>
            <p className="text-xs text-slate-400 mt-1 max-w-md mx-auto">
              There are no deleted tasks matching your search and filter criteria.
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {deletedTasks.map(task => {
              ...
              return (
                <motion.div
                  ...
                  initial={{ opacity: 0, scale: 0.98 }}
                  animate={{ opacity: 1, scale: 1 }}
                  className="bg-rose-50/10 border border-rose-150/40 rounded-xl p-4 shadow-3xs hover:shadow-2xs transition-all relative flex flex-col justify-between group"
                >
                  <div>
                    {/* Project and Discipline tag */}
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <div className="flex items-center gap-1.5">
                        <span className="px-1.5 py-0.5 rounded text-[8px] font-bold uppercase tracking-wider font-mono bg-rose-50 text-rose-600 border border-rose-100">
                          Deleted
                        </span>
                        <span className={`px-2 py-0.5 rounded text-[9px] font-bold uppercase tracking-wider font-mono border ${
                          task.type === 'architecture' ? 'bg-red-50 text-red-600 border-red-100' :
                    ...
                    <div
                      ...
                      className={`flex items-start justify-between gap-2 ${isMobile ? 'cursor-pointer select-none pb-1' : ''}`}
                    >
                      <h3 className="text-xs font-bold text-slate-900 group-hover:text-rose-600 transition-colors flex items-start gap-1.5 flex-1 leading-snug line-through decoration-rose-300">
                        <span className="text-slate-400 font-mono text-[10px] select-none">[{task.code}]</span>
                        <span>{task.title}</span>
                      </h3>
                      {isMobile && (
                  ...
                  {(!isMobile || expandedTasks[task.id]) && (
                    <div className="flex items-center justify-between border-t border-slate-100 pt-3">
                      ...
                      <div className="flex items-center gap-1.5">
                        <button
                          onClick={() => onToggleArchiveTask(task.id)}
                          className="px-2.5 py-1 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 rounded-lg text-[10px] font-bold flex items-center gap-1 transition-colors cursor-pointer border border-emerald-200/20"
                          title="Restore task back to live Kanban board"
                        >
                        ...
                        <button
                          onClick={() => onDeleteTask(task.id)}
                          className="p-1.5 rounded-lg border bg-rose-50 border-rose-100 text-rose-600 hover:bg-rose-100 cursor-pointer"
                          title="Permanently delete this task from the database"
                        >
                          <Trash2 className="w-3.5 h-3.5" />
                        </button>
                      </div>
